...
const useChatActions = () => {
  const { chatInputRef, getActiveConnection } = useStore()
  ...
  const updateConnectionHealth = useStore((state) => state.updateConnectionHealth)
  const [agentId, setAgentId] = useQueryState('agent')
  const [teamId, setTeamId] = useQueryState('team')
  const [, setDbId] = useQueryState('db_id')

  // Get endpoint from active connection
  ...
  const getStatus = useCallback(async () => {
    ...
    if (!endpoint) return 503

    try {
      const status = await getStatusAPI(endpoint)
      // Update connection health
      if (selectedConnectionId) {
        updateConnectionHealth(selectedConnectionId, status === 200)
      }
      return status
    } catch {
      if (selectedConnectionId) {
        updateConnectionHealth(selectedConnectionId, false)
      }
      return 503
    }
  }, [getEndpoint, selectedConnectionId, updateConnectionHealth])
  ...
  const getAgents = useCallback(async () => {
    ...
    try {
      const agents = await getAgentsAPI(endpoint)
      return agents
    } catch {
  ...
  const getTeams = useCallback(async () => {
    ...
    try {
      const teams = await getTeamsAPI(endpoint)
      return teams
    } catch {
