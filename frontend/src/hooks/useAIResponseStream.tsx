...
/**
 * Detects if the incoming data is in AGUI format
 * @param data - The parsed data object
 * @returns true if it's AGUI format
 */
function isAGUIFormat(data: RunResponseContent): boolean {
  return (
    typeof data === 'object' &&
    data !== null &&
    'event' in data &&
  ...
  while (jsonStartIndex !== -1 && jsonStartIndex < buffer.length) {
    ...
    if (jsonEndIndex !== -1) {
      ...
      try {
        ...
        } else if (isAGUIFormat(parsed)) {
          // AGUI format - convert to legacy format for compatibility
